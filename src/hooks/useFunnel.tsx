...
export const useFunnel = <T extends string>(steps: T[], initialStepIndex: number = 0) => {
	...
	const Funnel = ({ children }: FunnelProps<T>) => {
		const targetStep = children.find((childStep) => childStep.props.name === steps[currentIndex]);

		return <>{targetStep}</>;
	};
	const nextStep = () => {
		setCurrentIndex((prevIndex) => (prevIndex < steps.length - 1 ? prevIndex + 1 : prevIndex));
	};
