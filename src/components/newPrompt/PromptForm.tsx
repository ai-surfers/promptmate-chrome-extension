...
import { CreatePromptRequest, InputFormat } from '../../hooks/mutations/prompt/usePostPrompt';
...
export default function PromptForm({ onSubmit, initialData }: PromptFormProps) {
	...
	function handleHelp() {
		openUrlInNewTab('https://pocket-prompt.notion.site/da477857a0cc44888b06dd23cf6682ff');
	}

	return (
		<Form layout="vertical" requiredMark="optional" form={form} onFinish={handleOnFinish}>
			...
			<Form.Item name="categories" label="분야" rules={[{ required: true }]}>
				<Select placeholder="분야를 선택해주세요" allowClear mode="multiple">
			...
			<Form.Item
				...
				help={
					<Help onClick={handleHelp}>
						<QuestionCircleOutlined /> 어떻게 작성해야 할지 모르겠어요
					</Help>
				}
			>
				<TextArea
...
이들의 마음을 사로잡을 수 있는 매력적이고 센스있는 카피라이팅을 각기 다른 컨셉으로 총 3개 만들어줘"
			...
			<InputTags tags={inputs} onInsert={insertTag} onRemove={removeTag} />

			<Button type="primary" style={{ width: '100%', marginBottom: '50px' }} htmlType="submit">
				{initialData ? '수정' : '추가'}
			</Button>
		</Form>
	);
